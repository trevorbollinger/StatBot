...
function UserStatistics() {
  ...
  useEffect(() => {
    async function fetchStats() {
      try {
        const params = [];
        if (!includeBots) {
          params.push("exclude_bots=true");
          params.push("exclude_channel=bot-shit");
        }
        const queryString = params.length ? `?${params.join("&")}` : "";

        const [usersResponse, messageStatsResponse] = await Promise.all([
          api.get(`/api/stats/users${queryString}`),
          api.get(`/api/stats/message-stats${queryString}`),
        ]);
  ...
  }, [autoRefresh, refreshInterval, includeBots]);
  ...
  const calculateTotals = (users) => {
    const totals = users?.reduce(
      (acc, user) => ({
        ...
        messagePercentages:
          acc.messagePercentages +
          parseFloat(
            calculatePercentage(user.message_count, messageStats.total_messages)
          ),
        wordPercentages:
          acc.wordPercentages +
          parseFloat(
            calculatePercentage(user.total_words, messageStats.total_words)
          ),
        characterPercentages:
          acc.characterPercentages +
          parseFloat(
            calculatePercentage(
              user.total_characters,
              messageStats.total_characters
            )
          ),
      }),
      {
              ...
              <thead>
                <tr>
                  {[
                    ...
                    {
                      label: "CPM",
                      key: "characters_per_message",
                      style: { width: "70px" },
                    },
                    {
                      label: "WPM",
                      key: "words_per_message",
                      style: { width: "70px" },
                    },
                    {
                      label: "Most Active Channel",
                      key: "most_active_channel",
                    },
                  ].map(({ label, key, style }) => (
                    <th
                      key={key}
              ...
              <tbody>
                {sortedUsers().map((user) => (
                  ...
                  >
                    ...
                    <td>
                      {(user.total_words / (user.message_count || 1)).toFixed(
                        2
                      )}
                    </td>
                    <td>
                      {user.most_active_channel
                        ? `${user.most_active_channel} (${user.most_active_channel_percentage}%)`
                        : "N/A"}
                    </td>
                  </tr>
                ))}
                    ...
                    {(() => {
                      ...
                      return (
                        <>
                          <td>{`${formatNumber(
                            totals.messages
                          )} (${totals.messagePercentages.toFixed(1)}%)`}</td>
                          <td>{`${formatNumber(
                            totals.words
                          )} (${totals.wordPercentages.toFixed(1)}%)`}</td>
                          <td>{`${formatNumber(
                            totals.characters
                          )} (${totals.characterPercentages.toFixed(1)}%)`}</td>
                          <td>
                            {calculateAvgCharsPerMessage(
                              totals.characters,
                              totals.messages
