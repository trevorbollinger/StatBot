    ...
    async function fetchStats() {
      try {
        ...
        const [channelsResponse, messageStatsResponse] = await Promise.all([
          api.get(`/api/stats/channels${queryString}`),
              ...
              <tbody>
                {sortedChannels().map((channel) => (
                  <tr key={channel.channel_name}>
                    ...
                    <td>
                      {(
                        channel.total_words / (channel.message_count || 1)
                      ).toFixed(2)}
                    </td>
                    <td>
                      {channel.most_active_user
                        ? `${channel.most_active_user} (${channel.most_active_user_percentage}%)`
                        : "N/A"}
                    </td>
                  </tr>
                ))}
