...
import { calculateAvgCharsPerMessage, formatNumber, handleCopyWithToast } from "../utils.js";
    ...
    return (
        <div className="home-container fade-in">
            ...
            <div className="card-container">
                <div className="stat-card" onClick={() => handleStatClick(totalMessages)}>
                ...
                <div className="stat-card" onClick={() => handleStatClick(totalCharacters)}>
                    <div className="stat-label">Total Characters</div>
                    <div className="stat-number">{formatNumber(totalCharacters)}</div>
                </div>                
                <div className="stat-card" onClick={() => handleStatClick((totalMessages > 0 ? (totalWords / totalMessages).toFixed(1) : 0), true)}>
                    <div className="stat-label">Average Words / Message</div>
                    <div className="stat-number">
                        {(totalMessages > 0 ? (totalWords / totalMessages).toFixed(1) : 0)}
                    </div>
                </div>
                <div className="stat-card" onClick={() => handleStatClick(calculateAvgCharsPerMessage(totalCharacters, totalMessages), true)}>
                    <div className="stat-label">Average Chars / Message</div>
                    <div className="stat-number">
                        {calculateAvgCharsPerMessage(totalCharacters, totalMessages)}
                    </div>
                </div>
                <div className="stat-card" onClick={() => handleStatClick(messageStats?.most_active_day?.count)}>
                    <div className="stat-label">Most Active Day</div>
                    <div className="stat-number">
                        {messageStats?.most_active_day ? (
                            <>
                                {new Date(messageStats.most_active_day.date).toLocaleDateString()}:
                                <span className="stat-number"> {formatNumber(messageStats.most_active_day.count)}</span>
                            </>
                        ) : 'N/A'}
                    </div>
                </div>
                <div className="stat-card" onClick={() => handleStatClick(avgMessagesPerDay)}>
                    <div className="stat-label">Average Messages / Day</div>
                    <div className="stat-number">{formatNumber(avgMessagesPerDay)}</div>
                </div>
                <div className="stat-card" onClick={() => handleStatClick(messagesLast24Hours)}>
            ...
            <Graph
                data={timelineData}
                onPeakIntervalChange={setPeakInterval}
            />
                            ...
                            <div className="message-content-wrapper">
                                <div className="message-header">
                                    <div className="message-meta">
                                        ...
                                        <span>{formatNumber(msg.word_count)} words  •  {formatNumber(msg.char_count)} chars</span>
                                    </div>
                                    </div>
                                </div>
                                <div className="message-content-home">{msg.message_content}</div>
                            </div>
                        </div>
                    ))}
                </div>
