...
const Base = ({ children }) => {
  ...
  const { autoRefresh, setAutoRefresh, refreshInterval, setRefreshInterval } = useRefresh();
    ...
    <>
      ...
      {isAuthorized && (
        <div className={`sidebar ${isMobileMenuOpen ? 'open' : ''}`}>
          ...
          <Link to="/userstats" className={location.pathname === '/userstats' ? 'active' : ''} onClick={() => setIsMobileMenuOpen(false)}>
            User Statistics
          </Link>
          <Link to="/channelstats" className={location.pathname === '/channelstats' ? 'active' : ''} onClick={() => setIsMobileMenuOpen(false)}>
            Channel Statistics
          </Link>
          <Link to="/messagestats" className={location.pathname === '/messagestats' ? 'active' : ''} onClick={() => setIsMobileMenuOpen(false)}>
            Message Statistics
          </Link>
          <Link to="/database" className={location.pathname === '/database' ? 'active' : ''} onClick={() => setIsMobileMenuOpen(false)}>
            Database
          </Link>
      ...
      <nav className="navbar">
        ...
        {isAuthorized ? (
          <div className="navbar-right">
            ...
            <button onClick={handleLogout} className="button blue-button">Logout</button>
          </div>
        ) : (
          <></>
        )}
