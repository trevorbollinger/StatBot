      ...
      <table className="stats-table">
        <thead>
          <tr>
            ...
            <th onClick={() => sortBy("attachments")} className="sortable">
              📎 {getSortIcon("attachments")}
            </th>
            <th onClick={() => sortBy("mentions")} className="sortable">
              @ {getSortIcon("mentions")}
            </th>
            <th onClick={() => sortBy("emojis")} className="sortable">
              🦧 {getSortIcon("emojis")}
            </th>
            ...
            >
              ...
              <td>
                ...
                <span className="percentage">
                  (
                  {((item.message_count / totals.message_count) * 100).toFixed(
                    1
                  )}
                  %)
                </span>
              ...
              <td>
                ...
                <span className="percentage">
                  (
                  {(
                    (item.total_characters / totals.total_characters) *
                    100
                  ).toFixed(1)}
                  %)
                </span>
              ...
              <td>
                {showUserStats
                  ? item.most_active_channel
                  : item.most_active_user}
                <span className="percentage">
                  (
                  {(showUserStats
                    ? item.most_active_channel_percentage
                    : item.most_active_user_percentage
                  ).toFixed(1)}
                  %)
                </span>
              </td>
